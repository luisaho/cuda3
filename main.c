...
int main(int argc, char *argv[]){
	...
	//die Matrix wird in beiden Formaten immer im Arbeitsspeicher gehalten
	floatType* dataDIA = NULL;
	int* offsetsDIA = NULL;
	...
	if(MATRIXFORMAT==0) printf("Matrixformat: ELLPACK-R\n");
	else if(MATRIXFORMAT==1) printf("Matrixformat: CSR\n");
	else if(MATRIXFORMAT==2) printf("Matrixformat: Sliced pJDS\n");
	...
	if(MATRIXFORMAT == 0){			//ELLPACK-R
		gpuWarmup2();
		solveTime = getWTime();
		cg(n, nnz, maxNNZ, data, indices, length, b, x, &sc, 0);
		solveTime = getWTime()-solveTime;
	}else if(MATRIXFORMAT == 1){	//CSR
		floatType* dataCSR;
		int *indicesCSR, *lengthCSR;
		toCSR(data, indices, length, n, nnz, &dataCSR, &indicesCSR, &lengthCSR);
		gpuWarmup2();
		solveTime = getWTime();
		cg(n, nnz, maxNNZ, dataCSR, indicesCSR, lengthCSR, b, x, &sc, 0);
		solveTime = getWTime()-solveTime;
		destroyMatrix(dataCSR, indicesCSR, lengthCSR);
	}else if(MATRIXFORMAT == 2){	//sliced pJDS
		//permutation speichert Vertauschung der Zeilen
		int* permutation;
		permutation = malloc(n*sizeof(floatType));
		floatType* dataSPJDS;
		int *indicesSPJDS, *lengthSPJDS;
		toSPJDS(data, indices, length, n, maxNNZ, &dataSPJDS, &indicesSPJDS, &lengthSPJDS, permutation);
		gpuWarmup2();
		solveTime = getWTime();
		cg(n, nnz, maxNNZ, dataSPJDS, indicesSPJDS, lengthSPJDS, b, x, &sc, permutation);
		solveTime = getWTime()-solveTime;
		free(permutation);
		destroyMatrix(dataSPJDS, indicesSPJDS, lengthSPJDS);
	}
	...
	printf("bnrm2 = %f | residual = %f | sc.tolerance = %f\n",bnrm2, residual, sc.tolerance);
	...
	output(
	    ...
	    //"Hotspot GFLOP/s", 'f', ((2 * nnz * (sc.maxIter+1)) / (sc.timeMatvec * 1000000000)),
		"Hotspot GFLOP/s", 'f', ((2.0 * ((double)nnz) * ((double)(sc.iter))) / (sc.timeMatvec * 1000000000.0)),	//alles zu double casten => keine Probleme mehr mit Überläufen/Vorzeichen
	    "IO time", 'f', ioTime,
